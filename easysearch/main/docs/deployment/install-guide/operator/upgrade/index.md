...
如果集群启用了跨集群复制（CCR），跨版本滚动升级期间还需要额外关注复制任务的自动暂停与手工恢复要求。请同时参考 生产环境部署 中的“CCR 滚动升级注意事项”和 跨集群复制 文档。
...
如果集群启用了跨集群复制（CCR），跨版本滚动升级期间还需要额外关注复制任务的自动暂停与手工恢复要求。请同时参考 [生产环境部署]({{< relref "/docs/deployment/install-guide/production-env.md#ccr-滚动升级注意事项" >}}) 中的“CCR 滚动升级注意事项”和 [跨集群复制]({{< relref "/docs/operations/cluster-admin/ccr" >}}) 文档。
