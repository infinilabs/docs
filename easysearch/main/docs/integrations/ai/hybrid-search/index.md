...
title: "混合搜索"
date: 0001-01-01
summary: "混合搜索 #  文档中的 混合搜索（Hybrid） 只指本页的搜索管道方案：hybrid 查询加上 hybrid_ranker_processor（RRF）。 同一请求里用 bool 组合 query-level knn 和 match 是复合查询，不是 Hybrid。
...
文档中的 **混合搜索（Hybrid）** 只指本页的搜索管道方案：`hybrid` 查询加上 `hybrid_ranker_processor`（RRF）。
同一请求里用 `bool` 组合 query-level `knn` 和 `match` 是复合查询，不是 Hybrid。
...
执行混合搜索
以下示例请求组合了两个查询子句：[语义查询]({{< relref "/docs/integrations/ai/search-text-embedding.md" >}})和匹配查询。它使用上一步设置的默认搜索管道进行查询
 ```auto
