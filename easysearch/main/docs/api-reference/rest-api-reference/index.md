...
## replication.all_status

列出 follower 集群中所有复制索引的当前状态。
...
| 参数 | 类型 | 说明 |
| :----- | :--- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| status | list | 可选。按复制状态过滤 follower 索引。支持逗号分隔的多值，大小写不敏感。支持 `BOOTSTRAPPING`、`SYNCING`、`RUNNING`、`PAUSED`、`FAILED`、`STOPPED`。传入不支持的状态值会返回 `400 Bad Request`。 |
