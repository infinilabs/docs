...
title: "文档设计"
date: 0001-01-01
description: "单文档层面的建模：字段设计、多字段、标识符与路由、更新模式与并发控制。"
tags: ["最佳实践", "数据建模"]
summary: "文档设计 #  这一页回答两个问题：应该把什么放进一个文档？字段怎么设计才适合搜索？ 这里聚焦单个文档层面的建模，跨文档关系（Nested、Parent-Child、反范式）放在后续章节。
...
这一页回答两个问题：**应该把什么放进一个文档？字段怎么设计才适合搜索？** 这里聚焦单个文档层面的建模，跨文档关系（Nested、Parent-Child、反范式）放在后续章节。
...
在多租户/分库分表等场景下，可以考虑额外的路由字段（如 `tenant_id`），后续在索引设计和路由策略中使用（见 [多租户建模](./multi-tenancy.md)、[分布式写入]({{< relref "/docs/fundamentals/distributed-write.md" >}})）。
...
- 合理利用 multi-fields 同时支持全文检索与精确过滤
- 为标识符与路由预留好字段，方便后续扩展多租户策略
