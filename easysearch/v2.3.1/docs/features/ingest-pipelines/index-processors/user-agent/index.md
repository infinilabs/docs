...
{ &#34;user_agent&#34;: { &#34;field&#34;: &#34;user_agent&#34;, &#34;target_field&#34;: &#34;user_agent_info&#34; } } 配置参数 #  下表列出了 user_agent 处理器所需的和可选参数。
   参数 是否必填 描述     field 必填 包含用户代理字符串的字段。   target_field 可选 存储提取的用户代理信息的字段。如果未指定，则信息存储在 user_agent 字段中。   ignore_missing 可选 指定处理器是否应忽略缺失或值为 null 的 field。如果设置为 true ，则处理器在字段不存在或值为 null 时不会修改文档。默认为 false 。   regex_file 可选 自定义用户代理解析规则文件名。文件需位于 config/ingest-user-agent/ 目录下，且必须在节点启动时已存在；仅会加载 .yml 文件。如果未指定，则使用内置默认规则集。   properties 可选 要从用户代理字符串中提取并添加到 target_field 的属性列表。如果未指定，则默认启用全部属性。ecs=true 时常用输出为 original、name、version、os、device；ecs=false 时还会使用旧格式中的 major、minor、patch、build、os_name、os_major、os_minor 等字段。   ecs 可选 是否按 ECS 格式输出结果。默认为 true。设置为 false 时会使用旧的非 ECS 输出格式，该格式已被弃用。   description 可选 处理器的一个简要描述。   if 可选 处理器运行的条件。   ignore_failure 可选 指定处理器是否在遇到错误时继续执行。如果设置为 true ，则忽略失败。默认为 false 。   on_failure 可选 在处理器失败时运行的处理器列表。   tag 可选 处理器的标识标签。在调试中区分同一类型的处理器很有用。    如何使用 #  按照以下步骤在管道中使用处理器。"
---
...
| `ignore_missing` | 可选     | 指定处理器是否应忽略缺失或值为 `null` 的 `field`。如果设置为 true ，则处理器在字段不存在或值为 `null` 时不会修改文档。默认为 false 。                                                                      |
| `regex_file`     | 可选     | 自定义用户代理解析规则文件名。文件需位于 `config/ingest-user-agent/` 目录下，且必须在节点启动时已存在；仅会加载 `.yml` 文件。如果未指定，则使用内置默认规则集。                                           |
| `properties`     | 可选     | 要从用户代理字符串中提取并添加到 `target_field` 的属性列表。如果未指定，则默认启用全部属性。`ecs=true` 时常用输出为 `original`、`name`、`version`、`os`、`device`；`ecs=false` 时还会使用旧格式中的 `major`、`minor`、`patch`、`build`、`os_name`、`os_major`、`os_minor` 等字段。 |
| `ecs`            | 可选     | 是否按 ECS 格式输出结果。默认为 `true`。设置为 `false` 时会使用旧的非 ECS 输出格式，该格式已被弃用。                                                                                                       |
